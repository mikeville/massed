 ...
 */
function validate(input: unknown): ParsedWorkout {
  ...
  for (const raw of input.rows) {
    ...
    const weight =
      typeof raw.weight === 'number' && Number.isFinite(raw.weight) ? raw.weight : null;
    if (!name || !family || reps === null || weight === null) continue;
...
function isFamily(v: unknown): v is Family {
  return typeof v === 'string' && (FAMILY_VALUES as readonly string[]).includes(v);
}
